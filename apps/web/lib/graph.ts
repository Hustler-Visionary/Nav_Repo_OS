...
import { createRealRepoReadOnlyProvider } from "@tst-autonomous/repo-knowledge";
import { REPO_ROOT, DOMAIN_SCAN_ROOTS, UI_SCAN_ROOTS } from "./repo-config";
...
const provider = createRealRepoReadOnlyProvider(REPO_ROOT);
...
const buildGraphForRoots = async (scanRootsAbs: string[], options?: { excludeSegment?: string }): Promise<RepoGraph> => {
  let files: string[] = [];
  for (const root of scanRootsAbs) {
    try {
      files.push(...(await listTsFiles(root)));
    } catch {
      // a package with no such subtree (e.g. no presentation/ folder) -- skip it
    }
  }
  if (options?.excludeSegment) {
    files = files.filter((abs) => !toRelPath(abs).includes(`/${options.excludeSegment}/`));
  }
  ...
  for (const abs of files) {
    ...
    const layer = segments[segments.length - 2] ?? "domain";
  ...
  return { nodes, edges, scannedRoot: scanRootsAbs.map(toRelPath).join(", ") };
};

export const buildDomainGraph = (): Promise<RepoGraph> =>
  buildGraphForRoots(
    DOMAIN_SCAN_ROOTS.map((d) => path.join(REPO_ROOT, d)),
    { excludeSegment: "presentation" }
  );
export const buildComponentsGraph = (): Promise<RepoGraph> => buildGraphForRoots(UI_SCAN_ROOTS.map((d) => path.join(REPO_ROOT, d)));
