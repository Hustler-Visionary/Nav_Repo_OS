...
export const NodeDetailPanel = ({ node, onClose }: { node: GraphNode; onClose: () => void }) => {
  ...
  // Every package's UI-layer stub now lives under its own "presentation/"
  // subfolder (services/*/src/presentation, execution/*/src/presentation),
  // instead of one shared "src/components" root -- checking for that segment
  // works regardless of which package the node belongs to.
  const isUiNode = node.path.includes("/presentation/");
